...
const r = await fetch(https://api.clickup.com/api/v2/task/${taskId}, { headers: { Authorization: CLICKUP_API_KEY } });
return r.json();
}
async function copyFileInOneDrive(token, customerName) {
const t = await fetch(https://graph.microsoft.com/v1.0/users/${ONEDRIVE_USER_EMAIL}/drive/root:/${TEMPLATE_FOLDER}/${TEMPLATE_FILE_NAME}, { headers: { Authorization: Bearer ${token} } });
const td = await t.json();
const f = await fetch(https://graph.microsoft.com/v1.0/users/${ONEDRIVE_USER_EMAIL}/drive/root:/${DESTINATION_FOLDER}, { headers: { Authorization: Bearer ${token} } });
...
const nf = await fetch(https://graph.microsoft.com/v1.0/users/${ONEDRIVE_USER_EMAIL}/drive/root:/${DESTINATION_FOLDER}/${customerName}.xlsx, { headers: { Authorization: Bearer ${token} } });
